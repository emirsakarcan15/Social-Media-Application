...
function Comment({ setOpen, open, photoId }) {
  ...
  useEffect(() => {
    const startComments = async () => {
      const responseComments = await fetch(`http://localhost:3000/api/social/home/comment/${photoId}`, {
        method: "GET",
        headers: {
  ...
  const handleDeleteComment = async (id) => {
    const responseDeleteComment = await fetch(`http://localhost:3000/api/social/home/comment/${id}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  const handleCreateComment = async () => {
    ...
    const responseCreateComment = await fetch("http://localhost:3000/api/social/home/comment", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      credentials: "include",
      body: JSON.stringify({ photoId: photoId, text: newCommentText })
    })
